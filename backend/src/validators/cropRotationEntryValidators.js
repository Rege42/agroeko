...
export const createCropRotationEntryValidator = [
  ...
  body("seasonYear")
    ...
    .withMessage(
      "Поле seasonYear должно быть целым числом в диапазоне от 2000 до 2100",
    ),
  ...
  body("sowingDate")
    ...
    .isISO8601()
    .withMessage(
      "Поле sowingDate должно содержать корректную дату в формате ISO 8601",
    )
    .toDate(),
  ...
  body("harvestDate")
    ...
    .isISO8601()
    .withMessage(
      "Поле harvestDate должно содержать корректную дату в формате ISO 8601",
    )
    .toDate(),
  ...
  body("predictedYield")
    ...
    .isFloat({ min: 0 })
    .withMessage(
      "Поле predictedYield должно быть числом, большим или равным 0",
    ),
  ...
  body("seasonStatus")
    ...
    .withMessage(
      `Поле seasonStatus должно иметь одно из значений: ${SEASON_STATUSES.join(", ")}`,
    ),
  ...
  body("crop").custom(async (_, { req }) => {
    const { field: fieldId, crop: cropId } = req.body;
    ...
    if (
      ...
      throw new Error(
        "Выбранная культура не подходит для типа почвы данного поля",
      );
    }
...
export const updateCropRotationEntryValidator = [
  ...
  body().custom(async (value, { req }) => {
    ...
    const sowingDate = value.sowingDate
      ? new Date(value.sowingDate)
      : currentEntry.sowingDate;
    const harvestDate = value.harvestDate
      ? new Date(value.harvestDate)
      : currentEntry.harvestDate;
  ...
  body('crop').custom(async (value, { req }) => {
    const currentEntry = await CropRotationEntry.findById(
      req.params.id,
    ).populate("field crop");
...
export const listCropRotationEntriesValidator = [
  ...
  query("seasonYear")
    ...
    .isInt({ min: 2000, max: 2100 })
    .withMessage(
      "Параметр seasonYear должен быть целым числом в диапазоне от 2000 до 2100",
    ),
  ...
  query("seasonStatus")
    ...
    .isIn(SEASON_STATUSES)
    .withMessage(
      `Параметр seasonStatus должен иметь одно из значений: ${SEASON_STATUSES.join(", ")}`,
    ),
];
